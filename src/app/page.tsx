...
export default function Home() {
  ...
  const { data: categoryProducts, isFetching: isCategoryProductsFetching } =
    useGetProductsInCategoryQuery(selectedCategory!, {
      skip: !selectedCategory || selectedCategory === "All",
    });
  const { data: allProducts, isFetching: allProductsFetching } =
    useGetAllProductsQuery();
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5">
        {isCategoryProductsFetching || allProductsFetching
          ? Array.from(Array(8).keys()).map((e) => (
              <ProductCard.Skeleton key={e} />
            ))
          : (selectedCategory === "All" ? allProducts : categoryProducts)!.map(
